export async function loginUsuario(cedula, contrasena) {
  try {
    const res = await fetch("https://f127100235b2.ngrok-free.app/login", {
      method: "POST",
      headers: {
...
export async function obtenerVacaPorId(tamboId, vacaId) {
  try {
    const res = await fetch(`https://f127100235b2.ngrok-free.app/vaca/${tamboId}/${vacaId}`);
    ...
    if (data.ok) {
      ...
      return { ok: false, error: data.error || 'Animal no encontrado' };
    }
  ...
  export async function crearNota(vaca_id, contenido, motivo, token) {
    try {
      const res = await fetch('https://f127100235b2.ngrok-free.app/nota', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
