export async function loginUsuario(cedula, contrasena) {
  try {
    const res = await fetch("https://df2607f0f141.ngrok-free.app/login", {
      method: "POST",
      headers: {
...
export async function obtenerVacaPorId(tamboId, vacaId) {
  try {
    const res = await fetch(`https://df2607f0f141.ngrok-free.app/vaca/${tamboId}/${vacaId}`);
  ...
  export async function crearNota(vaca_id, contenido, motivo, token) {
    try {
      const res = await fetch('https://df2607f0f141.ngrok-free.app/nota', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
